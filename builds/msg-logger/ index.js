(function() {
    ...
    function SettingsComponent() {
        ...
        var C = {
            ...
            brand:"#5865f2", del:"#ed4245", edit:"#fee75c", editDark:"#b8960c",
        };

        var logs = (_storage && Array.isArray(_storage.logs)) ? _storage.logs : [];
        var counts = {
            all:  logs.length,
            del:  logs.filter(function(l){return l.type==="delete";}).length,
        ...
        var tabBar = h(RN.View,{style:{flexDirection:"row",marginHorizontal:12,marginTop:12,marginBottom:8,backgroundColor:C.bgDeep,borderRadius:10,padding:3}},
            [["All",counts.all,C.brand],["Deleted",counts.del,C.del],["Edited",counts.edit,C.edit]].map(function(pair,i){
                var label=pair[0],count=pair[1],accent=pair[2];
                var active=tab===i;
                return h(RN.TouchableOpacity,{key:i,onPress:function(){setTab(i);},activeOpacity:0.8,
                    style:{flex:1,alignItems:"center",paddingVertical:9,borderRadius:8,
                        backgroundColor:active?accent+(i===2?"44":"33"):"transparent",
                        borderWidth:active&&i!==0?1:0,borderColor:active?accent+"66":"transparent"}},
        ...
        var clearBtn = logs.length>0 && h(RN.TouchableOpacity,{onPress:doClear,activeOpacity:0.75,
            style:{marginHorizontal:12,marginBottom:10,backgroundColor:C.del+"18",borderRadius:8,paddingVertical:9,alignItems:"center",borderWidth:1,borderColor:C.del+"44"}},
            h(RN.Text,{style:{color:C.del,fontWeight:"600",fontSize:13}},"Clear "+filtered.length+" log"+(filtered.length!==1?"s":""))
        );
        ...
        var emptyView = h(RN.View,{style:{alignItems:"center",paddingTop:80}},
            ...
            h(RN.Text,{style:{color:C.muted,fontSize:13,marginTop:6,textAlign:"center",paddingHorizontal:30}},
                ...
                :"Send some messages so they get cached first.")
        );

        function renderEntry(entry,idx){
            var isDel  = entry.type==="delete";
            var accent = isDel?C.del:C.edit;
            var tag    = isDel?(entry.bulk?"BULK DEL":"DELETED"):"EDITED";
            return h(RN.TouchableOpacity,{key:entry.msgId+"_"+idx,onLongPress:function(){doCopy(entry);},activeOpacity:0.85,
                ...
                h(RN.View,{style:{padding:12}},
                    ...
                    h(RN.Text,{style:{color:C.muted,fontSize:11,marginBottom:10}},"#"+(entry.channelId||"?")+(entry.guildId?"  •  "+entry.guildId:"  •  DM")),
                    isDel
                        ? h(RN.View,{style:{backgroundColor:C.bgDeep,borderRadius:8,padding:10,borderLeftWidth:2,borderLeftColor:C.del+"88"}},
                            h(RN.Text,{style:{color:C.text,fontSize:14,lineHeight:20}},entry.content||"(empty)")
                          )
                        : h(RN.View,null,
                            h(RN.View,{style:{backgroundColor:C.bgDeep,borderRadius:8,padding:10,marginBottom:6,borderLeftWidth:2,borderLeftColor:C.del+"88"}},
                                h(RN.Text,{style:{color:C.del,fontSize:10,fontWeight:"700",marginBottom:4}},"BEFORE"),
                                h(RN.Text,{style:{color:C.text,fontSize:14,lineHeight:20}},entry.content||"(empty)")
                            ),
                            h(RN.View,{style:{backgroundColor:C.bgDeep,borderRadius:8,padding:10,borderLeftWidth:2,borderLeftColor:C.edit+"88"}},
                                h(RN.Text,{style:{color:C.editDark,fontSize:10,fontWeight:"700",marginBottom:4}},"AFTER"),
    ...
    return {
        onLoad: function() {
            ...
            var FD = vendetta.metro.common.FluxDispatcher;
            FD.subscribe("MESSAGE_CREATE",      onFlux);
            FD.subscribe("MESSAGE_UPDATE",      onFlux);
            FD.subscribe("MESSAGE_DELETE",      onFlux);
            FD.subscribe("MESSAGE_DELETE_BULK", onFlux);
            _unsubscribes = function() {
                FD.unsubscribe("MESSAGE_CREATE",      onFlux);
                FD.unsubscribe("MESSAGE_UPDATE",      onFlux);
                FD.unsubscribe("MESSAGE_DELETE",      onFlux);
                FD.unsubscribe("MESSAGE_DELETE_BULK", onFlux);
            };
        },
        onUnload: function() {
            if (_unsubscribes) { _unsubscribes(); _unsubscribes = null; }
